...
export default function ImageCompressor({
    ...
    formatName = null, // e.g., "PNG", "JPEG", "WebP" for format-specific pages
}) {
    ...
    // Server-side compression via API
    const compressImage = async (file, updateProgress) => {
        try {
            ...
            updateProgress("processing", "Compressing on server...");
            ...
            let statusMessage;
            if (reduction > 0) {
                const formatName = file.type === 'image/png' ? 'PNG' :
                    file.type === 'image/jpeg' ? 'JPG' : 'WebP';
                statusMessage = `${formatName} compressed`;
            } else {
                statusMessage = originalSize < 500 * 1024
                    ? "Already well optimized (small size)"
                    : "Already optimized";
            }
    ...
    const processFiles = async (fileList) => {
        ...
        const validFiles = Array.from(fileList).filter((file) => {
            const isValid = allowedFormats.includes(file.type);
            if (!isValid) {
                if (formatName) {
                    setErrorMessage(`This tool only accepts ${formatName} images. Please upload a ${formatName} file.`);
                } else {
                    setErrorMessage(`Unsupported file type: ${file.name}. Use PNG, JPG, or WebP.`);
                }
            }
            return isValid;
        });
        ...
        try {
            ...
            for (let i = 0; i < validFiles.length; i++) {
                ...
                // Show multi-file processing indicator
                ...
                try {
                    ...
                    let friendlyMessage = "Compression failed. Please try again.";
                    ...
                    if (error.message.includes('too large') || error.message.includes('10MB')) {
                        ...
                        friendlyMessage = `Compression failed for ${file.name}. Please try again.`;
                    }
                ...
                {files.length === 0 ? (
                    <div
                        ...
                        style={{
                            ...
                            background: processing ? "#f5f5f5" : "transparent",
                        }}
                    >
                        <input
                            ref={fileInputRef}
                        ...
                        <p style={{ fontSize: "1.25rem", fontWeight: 600, marginBottom: "0.5rem" }}>
                            {processing ? "Compressing..." : "Drop images here or click to select"}
                        </p>
                        ...
                        >
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    fontWeight: 600,
                                    opacity: (selectedIds.size === 0 || processing) ? 0.6 : 1,
                                }}
                            >
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                            {files.map((file) => (
                                <div
                                    key={file.id}
                                    style={{
                                        border: selectedIds.has(file.id)
                                        ...
                                        gap: "1rem",
                                        background: "white",
                                        cursor: file.status === "complete" ? "pointer" : "default",
                                        opacity: file.status === "complete" ? 1 : 0.7,
                                    }}
                                ...
                                >
                                        ...
                                        {file.status === "complete" ? (
                                            <>
                                                <p style={{ fontSize: "0.875rem", opacity: 0.7 }}>
                                                    ...
                                                    {file.reduction > 0 ? (
                                                        <span style={{ color: "#28a745", marginLeft: "0.5rem", fontWeight: 600 }}>
                                                            ({file.reduction}% smaller)
                                                        </span>
                                                    ) : (
                                                        <span style={{ color: "#6c757d", marginLeft: "0.5rem", fontWeight: 600 }}>
                                                            (No change)
                                                        </span>
                                                    )}
                                                </p>
                                            ...
                                            <p style={{ fontSize: "0.875rem", color: "#0070f3", fontWeight: 600 }}>
                                                {file.statusMessage}
                                            </p>
                                        )}
                                    </div>
                                    <button
                                        ...
                                        style={{
                                            padding: "0.5rem 1rem",
                                            background: (file.status === "complete" && !processing) ? "var(--primary)" : "#ccc",
                                            color: "white",
                                            border: "none",
                                            borderRadius: "6px",
                                            cursor: (file.status === "complete" && !processing) ? "pointer" : "not-allowed",
                                            fontWeight: 600,
                                            fontSize: "0.875rem",
                                        }}
                                    >
